...
// epipolar geometry constraint: m2^t*F*m1=0
    ...
    FileStorage fs(intriOutput, FileStorage::WRITE);
    if( fs.isOpened() )
    {
    ...
    fs.open(extriOutput, FileStorage::WRITE);
    if( fs.isOpened() )
    {
...
int main(int argc, char** argv)
{
    Size boardSize;
    string imagelistfn;
    string outputIntrinsics;
    string outputExtrinsics;
    bool showRectified;
    cv::CommandLineParser parser(argc, argv, "{w|9|}{h|6|}{s|1.0|}{nr||}{help||}{@input|data/stereo_calib.xml|}{@intrin|intrinsics.yml|}{@extrin|extrinsics.yml|}");
    if (parser.has("help"))
        return print_help();
    showRectified = !parser.has("nr");
    imagelistfn = parser.get<string>("@input");
    outputIntrinsics = parser.get<string>("@intrin");
    outputExtrinsics = parser.get<string>("@extrin");
    ...
    StereoCalib(imagelist, boardSize, squareSize, false, true, showRectified, outputIntrinsics, outputExtrinsics);
    return 0;
}
